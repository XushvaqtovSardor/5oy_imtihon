...
import { registerDto, UserRole, resetPasswordDto, LoginDto } from './dto/dto';
...
export class AuthService {
  ...
  async registerPhone(dto: registerDto) {
    if (!dto.phone) {
  ...
  async resetPassword(dto: resetPasswordDto) {
    ...
    const identifier = isEmail ? dto.email : dto.phone;
    ...
    const user = await this.prisma.users.findFirst({
      where: isEmail ? { email: dto.email } : { phone: dto.phone },
    });
